...
export function getStationFromZoneItem(s: { id: string; name: string; fill?: number; status: string; last?: string }, zoneName: string): Station {
  const cap = s.fill ?? 0
  const validStatus = (s.status === 'ACTIVE' || s.status === 'WARNING' || s.status === 'OFFLINE' || s.status === 'PENDING_ACTIVATION' || s.status === 'active' || s.status === 'offline' || s.status === 'warning') ? s.status : 'ACTIVE'
  return {
    id: s.id,
    name: s.name,
    location: `${zoneName}`,
