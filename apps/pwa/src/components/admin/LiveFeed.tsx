    ...
    async function loadFeed() {
      try {
        ...
        const mapped = res.data.map((evt: any) => {
          ...
          let matKey = 'plastic';
          if (cat.toLowerCase().includes('papel') || cat.toLowerCase().includes('carton') || cat.toLowerCase().includes('cartón'))
            matKey = 'paper';
          else if (
            cat.toLowerCase().includes('plastic') ||
            cat.toLowerCase().includes('plástic') ||
            cat.toLowerCase().includes('pet')
          )
            matKey = 'plastic';
          else if (
            cat.toLowerCase().includes('metal') ||
            cat.toLowerCase().includes('aluminio') ||
            cat.toLowerCase().includes('lata')
          )
            matKey = 'metal';
          ...
          return {
            ...
            time: new Date(evt.timestamp).toLocaleTimeString('es-ES', {
              hour: '2-digit',
              minute: '2-digit',
              second: '2-digit',
            }),
            ...
            return (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 10, color: 'rgba(240,253,244,0.32)', fontFamily: 'var(--font-mono)' }}>
                    {item.station}
                  </div>
                </div>
                <span
